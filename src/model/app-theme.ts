...
export function setAppTheme(preset: ThemePreset): void {
  ...
  current = {
    ...
    menuBorder: colorToken(
      tokens["desktop-menu-border"],
      "desktop-menu-border",
    ),
    menuHover: colorToken(tokens["desktop-menu-hover"], "desktop-menu-hover"),
    menuRadius: numberToken(
      tokens["desktop-menu-radius"],
      "desktop-menu-radius",
    ),
    trayBg:
    ...
    chromeFont: colorToken(
      tokens["desktop-chrome-font"],
      "desktop-chrome-font",
    ),
    taskbarHeight: numberToken(
      tokens["desktop-taskbar-height"],
    ...
    titlebarGradientTo:
      tokens["desktop-titlebar-gradient-to"] === undefined
        ? null
        : colorToken(
            tokens["desktop-titlebar-gradient-to"],
            "desktop-titlebar-gradient-to",
          ),
    ...
    bevel:
      tokens["desktop-bevel-light"] === undefined ||
      tokens["desktop-bevel-dark"] === undefined
        ? null
        : {
            lightOuter: colorToken(
              tokens["desktop-bevel-light"],
              "desktop-bevel-light",
            ),
            ...
            darkInner: colorToken(
              tokens["desktop-bevel-dark-inner"],
              "desktop-bevel-dark-inner",
            ),
            darkOuter: colorToken(
              tokens["desktop-bevel-dark"],
              "desktop-bevel-dark",
            ),
          },
    pinstripe:
      ...
      tokens["desktop-pinstripe-gap"] === undefined
        ...
        : {
            color: colorToken(
              tokens["desktop-pinstripe-color"],
              "desktop-pinstripe-color",
            ),
            gap: numberToken(
              tokens["desktop-pinstripe-gap"],
              "desktop-pinstripe-gap",
            ),
          },
    glow:
      tokens["desktop-glow-color"] === undefined ||
      tokens["desktop-glow-strength"] === undefined
        ? null
        : {
            color: colorToken(
              tokens["desktop-glow-color"],
              "desktop-glow-color",
            ),
            strength: numberToken(
              tokens["desktop-glow-strength"],
              "desktop-glow-strength",
            ),
          },
  };
}
...
function buttonShapeToken(value: unknown): TitlebarButtonShape {
  if (
    typeof value !== "string" ||
    !BUTTON_SHAPES.includes(value as TitlebarButtonShape)
  ) {
    throw new TypeError(
      `desktop-titlebar-button-shape must be one of ${BUTTON_SHAPES.join("|")}`,
    );
  }
...
function colorToken(value: string | number, name: string): string {
  if (typeof value !== "string")
    throw new TypeError(`${name} must be a color string`);
  return value;
}
...
function blendForContrast(
  ...
  for (let i = 0; i < 24; i++) {
    const mid = (lo + hi) / 2;
    if (contrastRatio(blend(foreground, background, mid), background) >= target)
      lo = mid;
    else hi = mid;
  }
